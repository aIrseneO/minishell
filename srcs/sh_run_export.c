...
/*   Updated: 2021/01/12 19:26:54 by atemfack         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	sh_export_repeated_keys(t_data *data, int i, int j, int n)
{
	char	*value;
...
static int		sh_load_new_envp(char **new_envp, char **envp, char **argv)
{
	int			i;
	char		*value;
...
int				sh_export(t_data *data)
{
	...
	if (sh_export_repeated_keys(data, 0, 0, 0) == -1)
	...
	if ((new_envp = ft_astrinit(size + 1)) == NULL)
		return (sh_perror_return("\x1B[31mMinishell: \x1B[0m", 
					"export", strerror(errno), 1));
	if (sh_load_new_envp(new_envp, data->envp, data->cmd[0]->argv) == -1)
		return (sh_perror_return("\x1B[31mMinishell: \x1B[0m",
                                        "export", strerror(errno), 1));
